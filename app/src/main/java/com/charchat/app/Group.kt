...
data class Group(
    ...
    fun toSystemPrompt(members: List<Character>): String = buildString {
        ...
        appendLine("- When a cue names a character, write ONLY that character's next line or action, in their own voice and first person, reacting to whatever just happened in the scene. Never write for any other character, and never write for the user - not their dialogue, their actions, or what happens to them (getting hurt, caught, changed, or anything else), not even in a fast-moving or dangerous moment. If the scene reaches a point where the user's character needs to act or something is about to happen to them, stop there and let the user say what happens. Never prefix the line with the character's name - just write their dialogue/actions directly.")
