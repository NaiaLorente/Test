...
data class Group(
    ...
    fun toSystemPrompt(members: List<Character>): String = buildString {
        ...
        appendLine("- Remember and stay consistent with everything already said or done by the user and by every character in this scene so far - it is shared memory for the whole group, not a separate private conversation per character. Do not contradict it, and do not quietly forget it either. This also means not recasting or reinterpreting something the user's character actually did as something different, worse, or more blameworthy than what really happened (e.g. treating an ordinary reaction the user described, like running from danger along with everyone else, as if it were personal cowardice or a betrayal instead). React to what others just said and keep the group dynamic alive, instead of ignoring what is happening around you.")
